<?php
//------------------
// shClient class provides data access and all operations withsettings, devices
//
// 
class naviClient {
	private $client_id = 0;
...
//		if (strlen($_safety_key) != 36) throw new Exception("Wrong safety key");
		...
		if (array_key_exists("dir", $settings)) {
			if (array_key_exists("products", $settings["dirs"])) $this->product_dir = $settings["dirs"]["products"];
			if (array_key_exists("orders", $settings["dirs"])) $this->product_dir = $settings["dirs"]["orders"];
			if (array_key_exists("routes", $settings["dirs"])) $this->product_dir = $settings["dirs"]["routes"];
		}
/*		var_dump($settings);
...
//		$this->dblink->close();
	...
	function getRoutes($_order) {
		if (!$z = simplexml_load_file('route-' . $_order . '.xml')) throw new Exception ("XML route-" . $_order . " is wrong!");
		$ret = array();
